...
fun main(args : Array<String>){
    ...
    agenda.removeAt(1)
    for (persoana in agenda){
        println(persoana.Name)
    }

    val contactDeCautat= Contact("Mihai", "0777", Birth(2004, 9, 17))
    println(contactDeCautat.Name)
    var isOk: Boolean = false
    for (persoana in agenda){
        if (persoana.Name === contactDeCautat.Name){
            println("Gasit")
            isOk = true
        }
    }
    if(isOk === false) println("Nu a fost gasit")
